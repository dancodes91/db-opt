...
var ZoomCustomizedResource = (function () {
  ...
  function init(opts) { 
    ...
    return {
       ...
       * @note You must call the function first before calling InitSDK(), or no, it won't work.
       */
      ...
      * @param {String} CustomizedStringType Specify the string item type.
      * @param {String} strCustomizedString Specify the custom string.
      * @return {Number} If the function succeeds, the return value is SDKERR_SUCCESS. Otherwise, this function returns an error.
      * @note If customizedString is null or is not null but with length zero(0), the return value is SDKERR_INVALID_PARAMETER.
      * @note You must call the function first before calling InitSDK(), or no, it won't work.
      */
       ...
       * @param {String} CustomizedURLType Specify the URL type.
       * @param {String} strCustomizeURL Specify the custom URL.
       * @return {Number} If the function succeeds, the return value is SDKERR_SUCCESS. Otherwise, this function returns an error.
       * @note If customizedUrl is null or is not null but with length zero(0), the return value is SDKERR_INVALID_PARAMETER.
       * @note You must call the function first before calling InitSDK(), or no, it won't work.
       */
